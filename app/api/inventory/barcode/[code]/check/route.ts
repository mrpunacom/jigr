...
export async function GET(
  ...
  try {
    ...
    const excludeItemId = url.searchParams.get('exclude') // Optional: exclude specific item when editing
    ...
    // Check for duplicate barcode (RLS will automatically filter by client_id)
    let query = supabase
      .from('inventory_items')
      .select('item_id, item_name, barcode, created_at')
